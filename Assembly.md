...
* **Power Supply:** 7.4 V (2S LiPo nominal). **DO NOT exceed 8.4 V — this servo is NOT 12 V tolerant.**
...
Power wiring (both servos share the same 2S LiPo):
  BLS3355 Red   → 7.4 V 2S LiPo +
  BLS3355 Black → GND (shared with Arduino GND)
...
| **Power (red)** | 2S LiPo + (NOT Arduino 5V) | 7.4 V, up to 6.2 A stall |
| **Ground (black)** | Arduino GND | Common ground |

> [!WARNING]
> **Do NOT power the BLS3355 from the Arduino's 5V pin.** The stall current (6.2 A at 8.4 V) would immediately damage the Arduino. Use a dedicated 2S LiPo with a shared ground.
